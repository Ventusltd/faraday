...
<div id="wrap"><div id="load" style="padding:18px 14px;color:#8aa">Opening: fetching the saved simulations (about 2.5 MB). On a slow connection this takes a few seconds.</div><canvas id="c"></canvas><div id="card"><span id="x">x</span><h2 id="h"></h2><div id="b"></div></div></div>
...
Promise.all([fetch('records.json').then(r=>r.json()),fetch('commands.json').then(r=>r.json())]).then(([rec,cfg])=>{R=rec.records;$('tally').textContent=rec.tally;$('load').remove();
